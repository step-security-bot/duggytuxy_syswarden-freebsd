syswarden_jail_drupal() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/drupal-auth.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/drupal-auth.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/drupal.conf
[drupal-auth]
enabled  = true
port     = http,https
filter   = drupal-auth
