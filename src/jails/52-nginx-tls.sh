syswarden_jail_nginx_tls() {
    ...
    if grep -qE "^\s*error_log\s+.*info;" "$NGINX_CONF" 2>/dev/null; then
        ...
        if nginx -t >/dev/null 2>&1; then
            if service nginx onestatus >/dev/null 2>&1 2>/dev/null; then
    ...
    if [[ ! -f "$NGINX_LOG" ]]; then
        touch "$NGINX_LOG"
        # Universal ownership fallback (FreeBSD)
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-tls-guard.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-tls-guard.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-tls-guard.conf
[syswarden-tls-guard]
enabled  = true
port     = https,443,8443
