syswarden_jail_silent_scanner() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-silent-scanner.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-silent-scanner.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-silent-scanner.conf
