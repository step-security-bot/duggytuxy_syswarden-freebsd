syswarden_jail_wordpress() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/wordpress-auth.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/wordpress-auth.conf
[Definition]
failregex = ^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"POST [^"]*?(?:wp-login\.php|xmlrpc\.php)[^"]*?" 200
ignoreregex = 
EOF
    fi

    # Write directly to jail.d using the dynamic centralized log path
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/wordpress.conf
[wordpress-auth]
enabled  = true
port     = http,https
