syswarden_jail_homoglyph() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-homoglyph.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-homoglyph.conf
[Definition]
failregex = ^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"(?:GET|POST|HEAD|PUT|PATCH|DELETE|OPTIONS) [^"]*(?:(?:(?:%%|\\x)F0(?:%%|\\x)9D(?:%%|\\x)9[0-9a-fA-F](?:%%|\\x)[89a-bA-B][0-9a-fA-F]){2,}|(?:(?:%%|\\x)E2(?:%%|\\x)80(?:%%|\\x)(?:8[b-fB-F]|A[a-eA-E])){2,})[^"]*" \d{3}
ignoreregex = 
EOF
    fi
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-homoglyph.conf
[syswarden-homoglyph]
enabled  = true
port     = http,https
