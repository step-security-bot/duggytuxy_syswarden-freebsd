syswarden_jail_webshell() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-webshell.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-webshell.conf
[Definition]
# Detects POST requests to upload/asset directories involving executable extensions
failregex = ^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"POST [^"]*(?:/upload|/media|/images|/assets|/files|/tmp|/wp-content/uploads)[^"]*\.(?:php\d?|phtml|phar|aspx?|ashx|jsp|cgi|pl|py|sh|exe)(?:\?[^"]*)? HTTP/[^"]*" \d{3}
ignoreregex = 
EOF
    fi
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-webshell.conf
[syswarden-webshell]
enabled  = true
port     = http,https
