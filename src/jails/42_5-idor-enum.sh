syswarden_jail_idor_enum() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-idor-enum.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-idor-enum.conf
[Definition]
# Detects rapid sequential or random access to sensitive endpoints resulting in 401/403/404
failregex = ^<HOST> \S+ \S+ (?:\[[^\]]*\]\s+)?"(?:GET|POST|HEAD|PUT|DELETE|PATCH) [^"]*(?:/api/v[0-9]+/|/users?/|/profile/|/invoices?/|/downloads?/|/docs?/|/id/|/view\?id=)[a-zA-Z0-9_-]+/?(?:[^"]*)? HTTP/[^"]*" (401|403|404)
ignoreregex = 
EOF
    fi
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-idor-enum.conf
[syswarden-idor-enum]
enabled  = true
port     = http,https
