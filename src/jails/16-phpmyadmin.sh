syswarden_jail_phpmyadmin() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/phpmyadmin-custom.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/phpmyadmin-custom.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/phpmyadmin.conf
[phpmyadmin-custom]
enabled  = true
port     = http,https
