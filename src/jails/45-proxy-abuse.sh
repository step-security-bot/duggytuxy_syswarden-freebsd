syswarden_jail_proxy_abuse() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-proxy-abuse.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-proxy-abuse.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-proxy-abuse.conf
