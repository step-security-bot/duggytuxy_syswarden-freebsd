syswarden_jail_cms_honeypot() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/syswarden-cms-honeypot.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/syswarden-cms-honeypot.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/syswarden-cms-honeypot.conf
