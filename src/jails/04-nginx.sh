syswarden_jail_nginx() {
    ...
    if [[ ! -f "/usr/local/etc/fail2ban/filter.d/nginx-scanner.conf" ]]; then
        cat <<'EOF' >/usr/local/etc/fail2ban/filter.d/nginx-scanner.conf
[Definition]
...
EOF
    ...
    cat <<EOF >/usr/local/etc/fail2ban/jail.d/nginx.conf
[nginx-http-auth]
enabled  = true
port     = http,https
