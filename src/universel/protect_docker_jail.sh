protect_docker_jail() {
    ...
    local action_file="/usr/local/etc/fail2ban/action.d/syswarden-docker.conf"
    if [[ ! -f "$action_file" ]]; then
        ...
        cat <<'EOF' >"$action_file"
[Definition]
actionstart = pf -N f2b-<name> 2>/dev/null || true
              pf -A f2b-<name> -j RETURN 2>/dev/null || true
              pf -I DOCKER-USER -j f2b-<name> 2>/dev/null || true
actionstop = pf -D DOCKER-USER -j f2b-<name> 2>/dev/null || true
             pf -F f2b-<name> 2>/dev/null || true
             pf -X f2b-<name> 2>/dev/null || true
actioncheck = pf -n -L DOCKER-USER | grep -q 'f2b-<name>[ \t]'
actionban = pf -I f2b-<name> 1 -s <ip> -j DROP
actionunban = pf -D f2b-<name> -s <ip> -j DROP
EOF
        chmod 644 "$action_file"
    fi
    # ------------------------------------------------------

    local jail_file="/usr/local/etc/fail2ban/jail.local"
    if [[ ! -f "$jail_file" ]]; then
    ...
    if command -v fail2ban-client >/dev/null && service fail2ban onestatus >/dev/null 2>&1; then
        local active_jails
    ...
    for raw_jail in "${jail_array[@]}"; do
        ...
        if [[ -f "/usr/local/etc/fail2ban/jail.d/${jail_name}.conf" ]]; then
            target_jail_file="/usr/local/etc/fail2ban/jail.d/${jail_name}.conf"
    ...
    if command -v pf >/dev/null && pf -n -L DOCKER-USER >/dev/null 2>&1; then
        while pf -D DOCKER-USER -m conntrack --ctstate ESTABLISHED,RELATED -j RETURN 2>/dev/null; do :; done
        pf -I DOCKER-USER 1 -m conntrack --ctstate ESTABLISHED,RELATED -j RETURN 2>/dev/null || true
        ...
        elif command -v service >/dev/null && [ -f /etc/init.d/pf ]; then
            service pf save 2>/dev/null || true
        fi
    fi
    # ------------------------------------------------------------
}
