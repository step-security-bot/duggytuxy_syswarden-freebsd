...
def monitor_logs():
    ...
    logs_to_tail = []
    for log_path in ['/var/log/kern-firewall.log', '/var/log/kern.log', '/var/log/messages', '/var/log/messages', '/var/log/fail2ban.log']:
    ...
    while True:
        if p.poll(100):
            ...
            if not line:
                print("[FATAL] Log process died or reached EOF. Exiting to allow rc.d to restart the service.", flush=True)
                break
...
EOF
        ...
        log "INFO" "Creating rc.d service for Reporter..."
        cat <<'EOF' >/usr/local/etc/rc.d/syswarden_reporter
#!/bin/sh
# PROVIDE: syswarden_reporter
# REQUIRE: DAEMON
# KEYWORD: shutdown

. /etc/rc.subr

name=syswarden_reporter
rcvar=syswarden_reporter_enable

command="/usr/sbin/daemon"
command_args="-f -S -T syswarden_reporter /usr/bin/env python3 /usr/local/bin/syswarden_reporter.py"

load_rc_config 
run_rc_command ""
EOF
        chmod +x /usr/local/etc/rc.d/syswarden_reporter
        sysrc syswarden_reporter_enable="YES"

        sysrc syswarden-reporter_enable=YES
        service syswarden-reporter start
