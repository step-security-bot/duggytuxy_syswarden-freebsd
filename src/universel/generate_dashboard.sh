generate_dashboard() {
    ...
    if service nginx onestatus >/dev/null 2>&1; then service nginx reload >/dev/null 2>&1 || true; fi
    if service apache2 onestatus >/dev/null 2>&1; then service apache2 reload >/dev/null 2>&1 || true; fi
    if service httpd onestatus >/dev/null 2>&1; then service httpd reload >/dev/null 2>&1 || true; fi

    # --- 2. CLOSE PORT 9999 ---
    log "INFO" "Securing perimeter: Closing UI Port 9999..."
    if command -v pf >/dev/null 2>&1; then
        pf delete allow 9999/tcp >/dev/null 2>&1 || true
    fi
    ...
    if command -v pf >/dev/null 2>&1; then
        pf -D INPUT -p tcp --dport 9999 -j ACCEPT 2>/dev/null || true
        ...
        if [[ -f "$DATA_FILE" ]]; then
            ...
            if [[ -n "$NEW_DATA" && "$NEW_DATA" != "$LAST_TELEMETRY_DATA" ]]; then
                ...
                if (( CURRENT_TS - LAST_GITHUB_TS >= 600 )) || [[ "$GH_STARS" == "--" ]]; then
                    GH_DATA=$(curl -s --max-time 1.2 https://api.github.com/repos/duggytuxy/syswarden-freebsd || echo "")
                    GH_REL_DATA=$(curl -s --max-time 1.2 https://api.github.com/repos/duggytuxy/syswarden-freebsd/releases/latest || echo "")
