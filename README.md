<p align="center">
  <a href="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/package.yml">
    <img src="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/package.yml/badge.svg" alt="SysWarden Builder and Packager">
  </a>
  <a href="https://github.com/duggytuxy/syswarden-freebsd/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/duggytuxy/syswarden-freebsd?logo=license" alt="GitHub License">
  </a>
  <img src="https://img.shields.io/badge/Platform-FreeBSD_14.4+-0052cc?logo=freebsd" alt="FreeBSD 14.4+">
  ...
  <a href="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/compliance.yml">
    <img src="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/compliance.yml/badge.svg?branch=main" alt="Plumber Compliance">
  </a>
  ...
  <a href="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/security-audit.yml">
    <img src="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/security-audit.yml/badge.svg" alt="SysWarden Security Audit">
  </a>
  <a href="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/dependabot/dependabot-updates">
    <img src="https://github.com/duggytuxy/syswarden-freebsd/actions/workflows/dependabot/dependabot-updates/badge.svg" alt="Dependabot Updates">
  </a>
...
* Injects Threat Intelligence directly into the `netdev` table under `pf` (or `raw PREROUTING` under `pf`). Malicious packets are destroyed right at the Network Interface Card (NIC), entirely bypassing kernel routing and the `conntrack` module to guarantee zero CPU impact during volumetric DDoS attacks.
...
* Implements pf-grade stateful enforcement by silently destroying late `FIN-ACK`/`RST` packets on expired `conntrack` sessions, and strictly blocking `NEW` connections lacking the `SYN` flag. This absolutely eradicates log pollution and false-positive portscan detections on active service ports, crucial for highly federated and mobile-heavy environments.
...
> [!IMPORTANT]
> SysWarden doesn't just stack firewall rules; it orchestrates the FreeBSD network stack to neutralize threats before they consume your resources:
...
4. The active defense layer analyzes application logs (via syslog) in real time. Any behavioral anomaly (brute-force, SQLi, LFI) triggers a surgical "AllPorts" ban that dynamically synchronizes the IP with the hardware drop tables.
...
SysWarden dynamically adapts to the native firewall orchestration engines of modern enterprise FreeBSD environments. The architecture relies on deep rc.d integration and natively binds to the following ecosystems:

| Operating System | Native Firewall Engine(s) Supported | Status |
| :--- | :--- | :--- |
| **FreeBSD 14.4+** | pf | Enterprise Ready |
...
### 2. Quick Installation (Package .pkg)

```bash
# Download the appropriate package for your distribution and its associated checksum file from the assets below
wget https://github.com/duggytuxy/syswarden-freebsd/releases/download/<version>/*.pkg
and
wget https://github.com/duggytuxy/syswarden-freebsd/releases/download/<version>/*.txt (SHA256SUMS)

# Verify Integrity
sha256sum -c SHA256SUMS.txt --ignore-missing

# For FreeBSD systems
pkg install -y ./syswarden-<version>.pkg
...
To learn everything about the SysWarden ecosystem, explore detailed configurations, and read advanced usage guides, visit our [official documentation page](https://github.com/duggytuxy/syswarden-freebsd/wiki/Deployment-Tutorial)
