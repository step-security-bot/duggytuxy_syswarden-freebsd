...
check_ip() {
    ...
    case "$FW_BACKEND" in
        ...
        firewalld)
            ...
            ;;
        pf)
            if pf -C pf-user-input -s "$target_ip" -j ACCEPT >/dev/null 2>&1 ||
                pf -C INPUT -s "$target_ip" -j ACCEPT >/dev/null 2>&1 ||
                ([[ "$target_ip" =~ : ]] && ip6tables -C pf-user-input -s "$target_ip" -j ACCEPT >/dev/null 2>&1); then
                is_whitelisted_fw=true
            fi
            ;;
        ipset | iptables | unknown)
            if [[ "$target_ip" =~ : ]]; then
                ...
                if pf -C INPUT -s "$target_ip" -j ACCEPT >/dev/null 2>&1; then is_whitelisted_fw=true; fi
            fi
            ;;
    esac
...
whitelist_ip() {
    ...
    case "$FW_BACKEND" in
        nftables)
            # Aligning with SysWarden v1.01.0 O(1) set-based whitelist architecture
            if [[ "$target_ip" =~ : ]]; then
        ...
        ipset | iptables | unknown)
            if [[ "$target_ip" =~ : ]]; then
                ...
                pf -I INPUT 1 -s "$target_ip" -j ACCEPT 2>/dev/null || true
                if command -v netfilter-persistent >/dev/null; then
                    netfilter-persistent save 2>/dev/null || true
                elif command -v /etc/init.d/pf >/dev/null; then
                    /etc/init.d/pf save 2>/dev/null || true
                fi
            fi
            ;;
    esac
        ...
        if [[ "$web_server" == "nginx" ]]; then
            ...
            if ! grep -q "Require ip $target_ip" "$web_conf"; then
                ...
                if { command -v apache2ctl >/dev/null 2>&1 && apache2ctl configtest >/dev/null 2>&1; } ||
                    ...
                    service "$web_server" reload >/dev/null 2>&1 || true
    ...
    case "$FW_BACKEND" in
        ...
        ipset | iptables | unknown)
            if [[ "$target_ip" =~ : ]]; then
                ...
                pf -D INPUT -s "$target_ip" -j ACCEPT 2>/dev/null || true
                if command -v netfilter-persistent >/dev/null; then
                    netfilter-persistent save 2>/dev/null || true
                elif command -v /etc/init.d/pf >/dev/null; then
                    /etc/init.d/pf save 2>/dev/null || true
    ...
    case "$FW_BACKEND" in
        ...
        ipset | iptables | unknown)
            if [[ "$target_ip" =~ : ]]; then
                ...
                pf -t raw -I PREROUTING 1 -p tcp -s "$target_ip" --dport "$SSH_PORT" -j ACCEPT 2>/dev/null || true
                if command -v netfilter-persistent >/dev/null; then
                    netfilter-persistent save 2>/dev/null || true
                elif command -v /etc/init.d/pf >/dev/null; then
                    /etc/init.d/pf save 2>/dev/null || true
    ...
    case "$FW_BACKEND" in
        ...
        ipset | iptables | unknown)
            if [[ "$target_ip" =~ : ]]; then
                ...
                while pf -t raw -D PREROUTING -p tcp -s "$target_ip" --dport "$SSH_PORT" -j ACCEPT 2>/dev/null; do :; done
                if command -v netfilter-persistent >/dev/null; then
                    netfilter-persistent save 2>/dev/null || true
                elif command -v /etc/init.d/pf >/dev/null; then
                    /etc/init.d/pf save 2>/dev/null || true
...
auto_whitelist_infra() {
    ...
    local gw_ips
    gw_ips=$(netstat -rn -f inet 2>/dev/null | awk '/^default/ {print $2}' || true)
    ...
    local dhcp_ips
    dhcp_ips=$(cat /var/db/dhclient.leases.* 2>/dev/null | grep -E 'option dhcp-server-identifier' | awk '{print $3}' | tr -d ';' | grep -Eo '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' || true)
    infra_ips="$infra_ips $dhcp_ips"

    # 5. Extract Host's own public/local IPs
    local host_ips
    host_ips=$(ifconfig 2>/dev/null | grep -oEo 'inet [0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | awk '{print $2}' | grep -v '^127\.' || true)
