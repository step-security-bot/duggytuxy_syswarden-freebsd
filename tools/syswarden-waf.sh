...
# Designed for SysWarden v1.01.1 Architecture
# Compatibility: FreeBSD 14.4+
...
install_dependencies() {
    ...
    # FreeBSD doesn't use legacy pcre, GeoIP, and yajl from standard repositories.
    ...
    pkg install -y gcc-c++ flex bison curl-devel curl doxygen zlib-devel \
    ...
    # 2. Geolocation Engine (libmaxminddb replaces legacy GeoIP in FreeBSD 14.4+/10)
    pkg install -y libmaxminddb-devel || pkg install -y GeoIP-devel || true

    # 3. Legacy PCRE (Safe fallback for older FreeBSD 14.4+ systems if pcre2 is not enough)
    pkg install -y pcre-devel || true

    # 4. JSON Support (YAJL) - Fallback to source compilation if natively missing
    if ! pkg install -y yajl yajl-devel; then
        log "WARN" "YAJL packages are missing from the package manager (expected on FreeBSD 14.4+0)."
        ...
        # YAJL installs to /usr/local/lib by default. FreeBSD ld.so does NOT
...
compile_apache_connector() {
    ...
    # Handle module loading based on OS architecture (FreeBSD)
...
EOF
        ...
        if $APACHE_DAEMON -t >/dev/null 2>&1; then
            ...
            #
            if service "$APACHE_DAEMON" status >/dev/null 2>&1; then
                service "$APACHE_DAEMON" restart
